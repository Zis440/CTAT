...
This document outlines the steps required to isolate the Super Admin functionality exclusively to the `admin.coretat.com` subdomain.
...
- **Hosting Dashboard**: Add the custom domain `admin.coretat.com` to your frontend project settings so that the platform recognizes and routes the domain to your existing application, and provisions an SSL certificate.

## 2. Frontend Routing Strategy

Since the frontend is a React SPA built with Vite, the same bundle is served on both `coretat.com` and `admin.coretat.com`. We use the `window.location.hostname` to conditionally mount routes.
...
const isAdminDomain = hostname === 'admin.coretat.com' || hostname === 'admin.localhost';
...
Add `https://admin.coretat.com` to your backend's CORS allowed origins. This is typically configured in `backend/.env` or directly where `CORSMiddleware` is initialized in `backend/app/main.py`.
