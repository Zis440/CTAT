...
class QuantitativeScorer:
    ...
    def score_story(self, story_text: str, events: list = None) -> Dict[str, Any]:
        ...
        # Complexity (heuristic based on sentence length and unique words)
        unique_words = len(set(w.lower() for w in words))
        complexity = min(100, (unique_words / word_count * 100) if word_count > 0 else 0)

        # If events are provided, compute conflict scores from them
        if events:
            num_events = max(len(events), 1)

            # --- Negative emotion labels that signal anxiety/conflict ---
            ...
            # Count events with negative valence OR negative emotions
            negative_events = 0
            ...
            # Internal conflict: negative valence + obstacle/defense event types
            ...
            # Interpersonal conflict: agent+target with negative valence/emotion
            ...
            # --- PRODUCTION CORRECTION: §3 Interpersonal Conflict Normalization ---
            # normalized_conflict = (weighted_sum / max_possible_weighted_sum) * 100
            raw_interpersonal = (interpersonal / num_events) * 80 + 5
            max_possible = 80 + 5  # max when all events are interpersonal conflicts
            conflict_interpersonal = min(100, (raw_interpersonal / max_possible) * 100)

            # Ego strength: average valence of events where agent is present
            ...
            # Anxiety level: combines negative valence intensity + anxiety-related emotions
            ...
            # Emotional stability: inverse of valence variance across events
            ...
            # --- PRODUCTION HARDENING: Peak Anxiety Index (§3) ---
            ...
            # --- PRODUCTION HARDENING: Emotional Volatility Index (§3) ---
            emotional_volatility_index = round_metric(compute_volatility(event_valences) * 100)

            # --- PRODUCTION HARDENING: Conflict Escalation Flag (§3) ---
            ...
            # --- PRODUCTION HARDENING: Emotional Transition Tracking (§4) ---
            ...
            # --- PRODUCTION HARDENING: Structured Narrative Protection (§4) ---
            # Prevent penalization of well-structured narratives

            # Reality testing: agent consistency and logical coherence
            ...
            # --- v3.2 Fix 4: Reality testing inflation cap ---
            # >90 only if: cognitive_complexity > 70, no distortions, multi-perspective
            if reality_testing_raw > 90:
                if complexity > 70 and unique_agents >= 2:
                    reality_testing = min(90, reality_testing_raw)  # cap at 90 (display 9.0)
                else:
                    reality_testing = min(88, reality_testing_raw)  # cap at 88 (display 8.8)
            else:
                reality_testing = reality_testing_raw

            # Narrative coherence: proportion of events with identifiable structure
            ...
            # --- FIX 8: Social Cognition & Self-Reflectiveness ---
            # Count explicit markers of meta-awareness, perspective monitoring, and internalized reflection
            ...
            # Baseline is drawn from internal reflective depth OR interpersonal interactions
            base_interpersonal = (interpersonal / max(1, num_events)) * 40
            
            # Calculate reflective bonus normalized securely 
            ...
            social_cognition = 30 # Default baseline without events

        # --- PRODUCTION HARDENING: Defaults for edge-case (no events) ---
        if not events:
        ...
        # --- PRODUCTION CORRECTION: §5 Affective Integration Recalibration ---
        affective_integration = compute_affective_integration(
            events if events else [], words
        )

        # Main quantitative scores dict
        scores_out = {
            ...
            # Production hardening fields (§3, §4)
        ...
        # --- Recalibration v3.0: Confidence intervals ---
        ...
        # --- Recalibration v3.0: Score distribution notes ---
        dist_notes = []
        if word_count < 50:
        ...
        # --- v3.2 Fix 4: Reality testing variance justification ---
