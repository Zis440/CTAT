...
interface SharedPatientDetailsProps {
  patientId: string;
  apiBasePath: string;
  backUrl: string;
  allowExport?: boolean;
...
export function SharedPatientDetails({ patientId, apiBasePath, backUrl, allowExport }: SharedPatientDetailsProps) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (form.living_condition !== (patient?.living_condition || "")) payload.living_condition = form.living_condition || null;
      if (form.family_structure !== (patient?.family_structure || "")) payload.family_structure = form.family_structure || null;
      if (form.environment_type !== (patient?.environment_type || "")) payload.environment_type = form.environment_type || null;
      if (form.education_level !== (patient?.education_level || "")) payload.education_level = form.education_level || null;
      if (form.occupation !== (patient?.occupation || "")) payload.occupation = form.occupation || null;
      if (form.socioeconomic_status !== (patient?.socioeconomic_status || "")) payload.socioeconomic_status = form.socioeconomic_status || null;
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, `Patient_${patient.id}.xlsx`);
            ...
            <CardContent className="space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-2">
                <div className="space-y-2">
                  ...
                  {isEditing ? (
                    <Select value={form.living_condition} onValueChange={(v) => setForm({ ...form, living_condition: v })}>
                  ...
                  ) : <p className="text-sm capitalize">{patient.living_condition || <span className="text-muted-foreground italic">—</span>}</p>}
                </div>
                <div className="space-y-2">
                  <Label className="text-muted-foreground font-bold text-xs uppercase tracking-wider">Family Structure</Label>
                  {isEditing ? (
                    <Select value={form.family_structure} onValueChange={(v) => setForm({ ...form, family_structure: v })}>
                      <SelectTrigger className="w-full bg-background/50"><SelectValue placeholder="Select" /></SelectTrigger>
                      <SelectContent>
                  ...
                  ) : <p className="text-sm capitalize">{patient.family_structure ? patient.family_structure.replace("_", " ") : <span className="text-muted-foreground italic">—</span>}</p>}
                </div>
                <div className="space-y-2">
                  <Label className="text-muted-foreground font-bold text-xs uppercase tracking-wider">Environment Type</Label>
                  {isEditing ? (
                  ...
                  ) : <p className="text-sm capitalize">{patient.environment_type ? patient.environment_type.replace("_", " ") : <span className="text-muted-foreground italic">—</span>}</p>}
                </div>
                <div className="space-y-2">
                  <Label className="text-muted-foreground font-bold text-xs uppercase tracking-wider">Education Level</Label>
                  {isEditing ? (
                  ...
                  ) : <p className="text-sm capitalize">{patient.education_level ? patient.education_level.replace("_", " ") : <span className="text-muted-foreground italic">—</span>}</p>}
                </div>
                <div className="space-y-2">
                  <Label className="text-muted-foreground font-bold text-xs uppercase tracking-wider">Occupation</Label>
                  {isEditing ? (
                    <Input value={form.occupation} onChange={(e) => setForm({ ...form, occupation: e.target.value })} placeholder="e.g. Student" className="w-full bg-background/50" />
                  ) : <p className="text-sm">{patient.occupation || <span className="text-muted-foreground italic">—</span>}</p>}
                </div>
                <div className="space-y-2">
                  <Label className="text-muted-foreground font-bold text-xs uppercase tracking-wider">Socioeconomic Status</Label>
                  {isEditing ? (
                    <Select value={form.socioeconomic_status} onValueChange={(v) => setForm({ ...form, socioeconomic_status: v })}>
                      <SelectTrigger className="w-full bg-background/50"><SelectValue placeholder="Select" /></SelectTrigger>
                      <SelectContent>
                  ...
                  ) : <p className="text-sm capitalize">{patient.socioeconomic_status || <span className="text-muted-foreground italic">—</span>}</p>}
                </div>
              </div>
        ...
        >
          <Card className="border-primary/10 bg-background/80 backdrop-blur-sm">
            <CardHeader>
              ...
              <CardDescription className="text-sm">
                Internal practitioner notes about this patient.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isEditing ? (
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      This will update the patient record for <strong>{displayName}</strong> with your changes.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel>Cancel</AlertDialogCancel>
