...
export function GenerateAnonymousLink() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (assessment) {
        const price = assessment.orgPrice || 0;
        if (price > 0) {
    ...
    <Card className="col-span-1 border-primary/10 bg-background/50 backdrop-blur-sm relative overflow-hidden mt-6">
      ...
      <CardHeader className="pb-3">
        <CardTitle className="text-lg flex items-center gap-2">
          <LinkIcon className="h-4 w-4 text-primary" /> Generate One-Time Link
        </CardTitle>
      ...
      <CardContent className="space-y-4">
        ...
        {generatedLink && (
          <div className="mt-4 p-3 rounded-md bg-muted/50 border animate-in fade-in">
            <label className="text-xs text-muted-foreground font-semibold mb-1 block">Your One-Time Link (Valid for 24 hours)</label>
