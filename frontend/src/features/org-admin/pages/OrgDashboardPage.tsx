...
import { Users, Wallet, CreditCard, Activity, TrendingUp, BarChart3, PieChart, UserPlus } from "lucide-react";
    ...
    <div className="space-y-6 w-full max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight">
            Welcome, {[user?.first_name, user?.last_name].filter(Boolean).join(" ") || "Organization Admin"}
          </h1>
          <p className="text-muted-foreground mt-1">Here is what's happening with your organization today.</p>
        </div>
        <div className="flex gap-2">
          <Button variant="default" className="gap-2 relative bg-primary/10 text-primary hover:bg-primary/20" asChild>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <BorderGlow className="hover:border-primary/30 transition-colors h-full">
            <Card className="border-0 bg-transparent shadow-none w-full h-full">
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">Total registered Candidates</p>
              </CardContent>
            </Card>
          </BorderGlow>
          ...
          <BorderGlow className="hover:border-primary/30 transition-colors h-full">
            <Card className="border-0 bg-transparent shadow-none w-full h-full">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-sm font-medium">Organization Staff</CardTitle>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-7">
        ...
        <Card className="col-span-1 lg:col-span-4 border-primary/10 bg-background/50 backdrop-blur-sm flex flex-col h-full overflow-hidden relative">
          ...
          <CardHeader className="pb-3">
            <CardTitle className="text-xl flex items-center gap-2"><TrendingUp className="h-5 w-5 text-muted-foreground" /> Candidate Growth ({new Date().getFullYear()})</CardTitle>
            <CardDescription>Monthly candidate acquisition overview</CardDescription>
          </CardHeader>
            ...
            {stats?.monthly_growth && stats.monthly_growth.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center opacity-50 space-y-4 py-8">
                ...
                <div className="text-center">
                  <p className="font-medium text-sm">No candidate data yet</p>
                  <p className="text-xs">Growth data will appear as candidates are registered.</p>
                </div>
              </div>
            )}
          </CardContent>
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" asChild className="text-xs h-8 bg-blue-500/10 text-blue-500 hover:bg-blue-500/20 shadow-none">
                <Link to="/org/transactions">View All</Link>
              </Button>
            </div>
          </CardHeader>
