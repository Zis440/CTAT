...
// ── Component ────────────────────────────────────────────────────────────────
export function OrgAppointmentDetailsPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();

  const [appt, setAppt] = useState<AppointmentAdmin | null>(null);
  ...
  useEffect(() => {
    ...
    fetchClinicAppointmentById(id)
      ...
      .catch(() => {
        ...
        navigate("/org/appointments");
      })
      .finally(() => setIsLoading(false));
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [id]);

  const syncForm = (data: AppointmentAdmin) => {
        ...
        >
          <Card className="border-primary/10 bg-background/80 backdrop-blur-sm h-full">
            <CardHeader>
              ...
              <CardDescription>Candidate and assigned psychologist.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
              ...
              <div className="flex items-center gap-3 p-3 rounded-xl border border-border/50 bg-muted/20">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] text-muted-foreground/70 uppercase tracking-wider mt-0.5">Candidate</p>
                </div>
              </div>
