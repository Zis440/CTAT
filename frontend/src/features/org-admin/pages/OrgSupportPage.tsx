...
export function OrgSupportPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!subject.trim() || !message.trim() || !supportType || !priority) {
      toast.error("Please fill in all required fields.");
      return;
    }
    setIsSubmitting(true);

    const finalSubject = `[${supportType.toUpperCase()}] ${subject}`;
    const finalMessage = `Priority: ${priority.toUpperCase()}\n\n${message}`;

    mutation.mutate({ subject: finalSubject, message: finalMessage });
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-5 flex-1 flex flex-col">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 shrink-0">
                    <div className="space-y-2.5">
                      <Label className="text-text/80 font-bold text-xs uppercase tracking-wider">Support Type</Label>
                      <Select required value={supportType} onValueChange={setSupportType}>
                        <SelectTrigger className="bg-background/50 border-primary/15 focus:border-primary/40"><SelectValue placeholder="Select type" /></SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="space-y-2.5">
                      <Label className="text-text/80 font-bold text-xs uppercase tracking-wider">Priority</Label>
                      <Select required value={priority} onValueChange={setPriority}>
                        <SelectTrigger className="bg-background/50 border-primary/15 focus:border-primary/40"><SelectValue placeholder="Select priority" /></SelectTrigger>
                        <SelectContent>
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card className="border-primary/10 bg-background/80 backdrop-blur-sm h-full flex flex-col shadow-xl shadow-primary/5 overflow-hidden pt-0 gap-0">
              <CardHeader className="bg-muted/20 border-b-0 shrink-0 pt-5 pb-5 sm:h-[104px] flex flex-col justify-center w-full">
