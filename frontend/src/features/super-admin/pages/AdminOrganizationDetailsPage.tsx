...
export function AdminOrganizationDetailsPage() {
  ...
  const loadData = async () => {
    ...
    setIsLoading(true);
    try {
      const data = await fetchAdminOrganization(id);
      setClinic(data);
      syncForm(data);
    } catch (err: any) {
  ...
  const syncForm = (data: AdminClinic) => {
    setOrganizationName(data.clinic_name || data.name || "");
    setEmail(data.email || "");
    setPhone(data.phone || "");
    setAddress(data.address || "");
    setRocNumber(data.roc_number || "");
    setOrganizationType(data.clinic_type || "");
  ...
  const handleSave = async () => {
    ...
    try {
      const payload: Partial<AdminClinic> = {
        clinic_name: organizationName.trim(),
        email: email.trim(),
        phone: phone.trim() || undefined,
        address: address.trim() || undefined,
        roc_number: rocNumber.trim() || undefined,
        clinic_type: organizationType.trim() || undefined,
  ...
  if (isLoading) {
    return (
      <div className="w-full relative min-h-full isolate">
        <Helmet>
          <title>Organization Details  | PsyicHub - Psychological Intelligence</title>
        </Helmet>
  ...
  const displayName = organizationName || clinic.clinic_name || clinic.name || "Unnamed Organization";
            ...
            <Card className="border-primary/10 bg-background/80 backdrop-blur-sm min-h-full">
              <CardHeader>
                ...
                <CardDescription className="text-sm">
                  Organization classification and active communication endpoints.
                </CardDescription>
              </CardHeader>
          ...
          >
            <Card className="border-primary/10 bg-background/80 backdrop-blur-sm min-h-full">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <Label className="text-muted-foreground font-bold text-xs uppercase tracking-wider flex items-center gap-1.5">
                    <Activity className="h-3.5 w-3.5" /> Organization Type
                  </Label>
                  {isEditing ? (
                    <Input
                    ...
                    <p className="text-sm font-semibold pt-1 text-foreground">
                      {organizationType || <span className="text-muted-foreground italic">Not specified</span>}
                    </p>
                  )}
                </div>
        ...
        {isEditing && (
          ...
          >
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>Save Organization Changes?</AlertDialogTitle>
                  <AlertDialogDescription>
