    ...
    <div className="space-y-6 w-full max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">View and manage all assessment reports across the platform.</p>
        </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between w-full">
              <div className="flex w-full max-w-sm items-center space-x-2">
                <SearchInput
                  ...
                  placeholder="Patient / Candidate Name"
                  onChange={(e) => {
                    ...
                    <div className="grid gap-4">
                      <div className="flex items-start justify-between">
                        <div className="space-y-2">
                          ...
                          <p className="text-sm text-muted-foreground">Adjust filters for reports.</p>
                        </div>
          ...
          <Table className="max-md:block">
            <TableHeader className="max-md:hidden">
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="font-semibold">Patient / Candidate Name</TableHead>
              ...
              {isLoading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <TableRow key={i} className="max-md:block max-md:p-4 max-md:border-b">
                    ...
                    <TableCell className="max-md:flex max-md:justify-between max-md:items-center max-md:p-1 max-md:border-none">
                      <span className="md:hidden font-semibold text-muted-foreground text-xs uppercase tracking-wider">Patient / Candidate Name</span>
                      <div className="flex items-center gap-3">
                ...
                paginatedReports.map((report) => (
                  <TableRow key={report.id} className="hover:bg-primary/5 transition-colors max-md:block max-md:p-4 max-md:border-b max-md:relative">
                    ...
                    <TableCell className="py-3 text-xs max-md:flex max-md:justify-between max-md:items-center max-md:p-1 max-md:border-none">
                      <span className="md:hidden font-semibold text-muted-foreground text-xs uppercase tracking-wider">Assessment</span>
                      <span className="truncate max-w-[200px] inline-block" title={getAssessmentName(report)}>{getAssessmentName(report)}</span>
                    </TableCell>
                    <TableCell className="py-3 text-xs text-muted-foreground max-md:flex max-md:justify-between max-md:items-center max-md:p-1 max-md:border-none">
                      ...
                      <div className="flex items-center text-muted-foreground gap-1.5 font-medium">
                        ...
                        <span className="truncate max-w-[120px]" title={report.psychologist_name || "Unknown"}>{report.psychologist_name || "Unknown"}</span>
                      </div>
                    </TableCell>
                    <TableCell className="py-3 text-xs text-muted-foreground max-md:flex max-md:justify-between max-md:items-center max-md:p-1 max-md:border-none">
                    ...
                    <TableCell className="py-3 text-right max-md:flex max-md:justify-between max-md:items-center max-md:p-1 max-md:border-none max-md:mt-2">
                      ...
                      <div className="flex items-center justify-end gap-2 flex-nowrap shrink-0">
                        <InteractiveHoverButton
                          ...
                          className="text-[11px] h-7 py-0 px-3"
                        >
                          Details
                        </InteractiveHoverButton>
                        {report.pdf_filename && (
                          <Button variant="outline" size="sm" className="h-7 w-7 p-0" onClick={() => handleDownloadPDF(report.id, report.test_type, report.pdf_filename!)} title="View PDF">
                            <FileText className="h-3.5 w-3.5" />
                          </Button>
                        )}
                        <Button variant="outline" size="sm" className="h-7 w-7 p-0" onClick={() => handleOpenLogs(report.id)} title="View Logs">
