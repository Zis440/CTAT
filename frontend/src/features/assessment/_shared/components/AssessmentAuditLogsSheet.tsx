...
export function AssessmentAuditLogsSheet({
  assessmentId,
  open,
  onOpenChange,
}: {
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-xl w-full overflow-y-auto overflow-x-hidden border-l border-border/50 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <SheetHeader className="text-left">
          ...
          <SheetDescription>
            A chronological list of actions performed for this assessment.
          </SheetDescription>
        </SheetHeader>
        <div className="mt-4 space-y-4">
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              <p>No audit logs found for this assessment.</p>
            </div>
          ) : (
                        ...
                        {Object.entries(log.details).map(([key, value]) => (
                          <div key={key} className="grid grid-cols-[120px_1fr] items-start gap-2">
                            <span className="font-medium text-muted-foreground">
                              {key === "clinic_or_org_name" && String(value) === "Independent Psychologist"
                                ? "Account Type:"
                                : key.replace(/_/g, " ")
