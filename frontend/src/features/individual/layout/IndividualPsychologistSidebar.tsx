...
import { LogOut, Moon, Sun, Wallet, Settings, Bell } from "lucide-react";
...
import { INDIVIDUAL_NAV } from "./sidebarConfig";
...
export function IndividualPsychologistSidebar() {
  ...
  const isActive = (to: string) => {
    ...
    if (to === "/wallet") return currentPath === "/wallet";
    ...
    return (
      <SidebarMenuItem key={itemId}>
        <SidebarMenuButton
          ...
          onClick={handleNavClick}
          className={cn(
            "h-10 rounded-xl font-semibold text-sm transition-all duration-150 relative",
            isActive(item.to)
              ? "bg-primary/15 text-primary"
        ...
        {INDIVIDUAL_NAV.map((group, idx) => {
          ...
          return (
            <div key={idx}>
              ...
              {idx < INDIVIDUAL_NAV.length - 1 && group.label && (
              ...
              >
                ...
                <DropdownMenuItem asChild className="cursor-pointer group" onClick={handleNavClick}>
                  <Link to="/notifications" className="flex items-center w-full">
                    <div className="w-8 flex items-center justify-start">
                      <Bell className="h-4 w-4 text-muted-foreground group-hover:text-white dark:group-hover:text-black group-focus:text-white dark:group-focus:text-black transition-colors" />
                    </div>
                    <div className="flex items-center gap-2">
