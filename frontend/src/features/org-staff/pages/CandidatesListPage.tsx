...
export function CandidatesPage() {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (currentPage !== 1) navigate('/org-staff/candidates');
  };
      ...
      <div className="space-y-6 w-full max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1">View and manage all candidate records.</p>
          </div>
                ...
                <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
                  <DialogTrigger asChild>
                    <Button type="button" variant="default" className="bg-primary/10 text-primary hover:bg-primary/20">
                      <ClipboardPlus className="mr-2 h-4 w-4" /> Add Candidate
                    </Button>
                  </DialogTrigger>
                  ...
                  Array.from({ length: 5 }).map((_, i) => (
                    <TableRow key={i} className="max-md:block max-md:p-4 max-md:border-b">
                      ...
                      <TableCell className="max-md:flex max-md:justify-between max-md:p-1 max-md:border-none">
                        <span className="md:hidden font-semibold text-muted-foreground">Candidate:</span>
                      ...
                      <TableRow key={patient.id} className="max-md:block max-md:p-4 max-md:border-b max-md:relative">
                        ...
                        <TableCell className="py-2 text-right max-md:flex max-md:justify-between max-md:items-center max-md:p-1 max-md:border-none max-md:mt-2">
                          ...
                          <InteractiveHoverButton
                            onClick={() => navigate(`/org-staff/candidates/${patient.id}/details`)}
        ...
        {totalPages >= 0 && !isLoading && (
          <div className="pt-6 pb-10 flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-xs text-muted-foreground w-full sm:w-auto text-center sm:text-left">
              Showing <span className="font-medium text-foreground">{filteredPatients.length === 0 ? 0 : (currentPage - 1) * ITEMS_PER_PAGE + 1}</span> to <span className="font-medium text-foreground">{Math.min(currentPage * ITEMS_PER_PAGE, filteredPatients.length)}</span> of <span className="font-medium text-foreground">{filteredPatients.length}</span> candidates
            </p>
            <Pagination className="sm:justify-end sm:w-auto mx-0">
              <PaginationContent>
                <PaginationItem>
                  <PaginationPrevious
                    onClick={() => navigate(currentPage > 2 ? `/org-staff/candidates/page/${currentPage - 1}` : `/org-staff/candidates`)}
                  ...
                  return (
                    <PaginationItem key={pageNum}>
                      <PaginationLink
                        ...
                        onClick={() => navigate(pageNum === 1 ? `/org-staff/candidates` : `/org-staff/candidates/page/${pageNum}`)}
                ...
                <PaginationItem>
                  <PaginationNext
                    onClick={() => navigate(`/org-staff/candidates/page/${currentPage + 1}`)}
